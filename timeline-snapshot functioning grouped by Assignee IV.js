...
//    console.log("startdates:" + d.newStartDateScript);
 ...
 //    return d.UniqueReference["#text"] + "-" + d.WebName["#text"];
   ...
   .attr("shortTitle", function (d) {
     return d.UniqueReference["#text"] + "-" + d.projectTitle.split(' ').slice(0,2).join('+');
   })
   .attr("class", "barsText")
     .attr("rx", 6)
     .attr("ry", 6)
...
function preventTooSmallWeekIntervals(period) {
        var tickSeconds = (xAxisWeek.ticks()[1] - xAxisWeek.ticks()[0]) / 1000;
...
//    d3.select(this).select('input').prop('checked','true');
...
d3.select("#collisionBtn").on("click", function(){
  ...
  .each(function(d, i) {
    ...
    mainObjNode = d3.select(this).node();
      ...
      var k=0;
        ...
        while (k < size) {
          k++;
        //  console.log("hittest:" + simpleCollisionDetection(mainObj, i));
          //  console.log("hittest:" + simpleCollisionDetection(mainObj, i));
